...
import com.kuplay.pi_framework.piv8.*;
...
public class V8Executor extends Thread {
    ...
    public void run() {
        synchronized (this) {
            ...
            runtime.registerJavaMethod(new ExecutorTermination(), "__piv8__checkThreadTerminate");
            setup(runtime);
        }
        try {
            if (!forceTerminating) {
                Object scriptResult = runtime.executeScript("__piv8__checkThreadTerminate();\n" + script, getName(), -1);
                if (scriptResult != null) {
                    result = scriptResult.toString();
                }
                if (scriptResult instanceof Releasable) {
                    ((Releasable) scriptResult).close();
                }
            }
