...
pi_modules.butil.exports = (function () {
	...
	module.utf8Decode = function (arr) {
...
pi_modules.depend.exports = (function () {
	...
	module.httpDomains = winit.httpDomains;
...
pi_modules.store.exports = (function () {
	 ...
	 * 删除指定的数据库
	 */
	module.deleteDatabase = function (db, success, fail, complete) {
		...
		var name = db.tabName;
		JSVM.store.delete(name, success, (e) => {
			onerror(fail, ERR_DELETE, e);
		}, complete);
	}


	module.create = function (dbName /*:string*/, tabName /*:string*/) {
	 ...
	 * @example
	 */
	module.init = function (db, success, fail, complete) { // 返回值类型1|类型2
		if(success === undefined){
			success = ()=>{}
		}
		if(fail === undefined){
		...
		var name = db.tabName;
		JSVM.store.create(name, success, (e) => {
			onerror(fail, module.ERR_INIT, e);
		}, complete);
	};
	 ...
	 * @example
	 */
	module.read = function (db, key, success, fail, complete) {
		if(success === undefined){
			success = ()=>{}
		}
		if(fail === undefined){
		...
		var onsuccess = (dh) => {
			success(dh.getContent())
		}
		module.sread(db, key, onsuccess, fail, complete);
	};

	module.sread = function (db, key, success, fail, complete) {
		if(success === undefined){
			success = ()=>{}
		}
		if(fail === undefined){
		...
		var name = db.tabName;
		JSVM.store.read(name, key, success, (e) => {
			onerror(fail, module.ERR_READ, e);
		}, complete);
	};
	 ...
	 * @example
	 */
	module.write = function (db, key, data, success, fail, complete) {
		if(success === undefined){
			success = ()=>{}
		}
		if(fail === undefined){
		...
		var name = db.tabName;
		JSVM.store.write(name, key, data, success, (e) => {
			onerror(fail, module.ERR_WRITE, e);
		}, complete);
	};
	 ...
	 * @example
	 */
	module.delete = function (db, key, success, fail, complete) {
		if(success === undefined){
			success = ()=>{}
		}
		if(fail === undefined){
		...
		var name = db.tabName;
		JSVM.store.remove(name, key, success, (e) => {
			onerror(fail, module.ERR_DELETE, e);
		}, complete);
	};
	 ...
	 * @example
	 */
	module.iterate = function (db, success, fail, complete) {
		if(success === undefined){
			success = ()=>{}
		}
		if(fail === undefined){
		...
		var name = db.tabName;
		JSVM.store.iterate(name, (data) => {
			var db = JSON.parse(data);
			for (var r in db) {
				if (success(r, db[r]) !== false)
					continue;
			}
		}, (e) => {
			onerror(fail, module.ERR_ITERATE, e);
		}, complete);
	};
	 ...
	 * @example
	 */
	module.clear = function (db, success, fail, complete) {
		if(success === undefined){
			success = ()=>{}
		}
		if(fail === undefined){
		...
		var name = db.tabName;
		JSVM.store.delete(name, () => {
			JSVM.store.create(name, success, () => { }, complete)
		}, (e) => {
			onerror(fail, module.ERR_CLEAR, e);
		}, complete);
	};

	// ============================== 本地
	// 返回错误处理函数
	var onerror = function (cb, type, e) {
		console.log(e);
		if (cb) {
			cb({
				error: type,
...
pi_modules.ajax.exports = (function () {
	...
	module.REQ_TYPE_BIN = 'bin';
	module.REQ_TYPE_JSON = 'json';
	module.REQ_TYPE_TEXT = 'text';
	 ...
	 * @example
	 */
	module.get = function (url /*:string*/, headers /*:join*/, reqData /*:json|string*/, reqType, respType /*:string*/, callback /*:function*/, errorCallback /*:function*/, processCallback /*:function*/) {
		var onsuccess = function (data) {
			if (respType === module.RESP_TYPE_BIN) {
				callback(base64js.toByteArray(data));
			} else {
				callback(data)
			}
		}
		return module.request('GET', url, headers, reqData, reqType, respType, onsuccess, errorCallback, processCallback);
	}
	...
	module.post = function (url /*:string*/, headers /*:join*/, reqData /*:string|ArrayBuffer|FormData*/, reqType, contentType /*:string*/, respType /*:string*/, callback /*:function*/, errorCallback /*:function*/, processCallback /*:function*/) {
		...
		return module.request('POST', url, headers, reqData, reqType, respType, onsuccess, errorCallback, processCallback);
	}
	 ...
	 * @example
	 */
	module.request = function (type /*:string*/, url /*:string*/, headers /*:join*/, reqData /*:string|ArrayBuffer*/, reqType, respType /*:string*/, callback /*:function*/, errorCallback /*:function*/, processCallback /*:function*/) {
		...
		return JSVM.request(type, url, headers, reqData, reqType, respType, callback, errorCallback, processCallback);
	}
...
pi_modules.load.exports = (function () {
	...
	module.deleteStore = function (cb) {
		store.deleteDatabase(localStore, cb);
	}
	...
	var localInit = function (cb) {
		...
		store.init(localStore, function () {
			store.read(localStore, "", function (value) {
				...
				var r = JSVM.Boot.loadJS(depend.domains[0], depend.rootPath() + "depend").getContent();
				localSign = {};
				var i = r.indexOf("["),
					j = r.lastIndexOf("]"),
					info;
			...
			}, console.log("localInit store read complete:123"))
		});
	};
	// 本地加载检查
	var localInitCheck = function (save /*:boolean*/) {
		...
		if (save) {
			store.write(localStore, "", JSON.stringify(localSign));
		}
	...
	var startURL = function (load /*:Load*/, durl, furl, files) {
		...
		var onsuccess = function () {
			count--;
			console.log("saveCount is :",count);
			if (count < 0) {
				store.write(localStore, "", JSON.stringify(localSign));
			}
			...
			console.log(load.saveCount,"=================",load.downAmount);
			if (load.saveCount >= load.downAmount) {
				load.onSaveSuccess(load.fileMap);
			}
		};
		
		JSVM.Boot.updateDownload(depend.httpDomains, url, files, function (file, dh) {
			var name = file.path;
			var hash = file.sign;
			var data = dh;
			localSign[name] = hash;
			store.write(localStore, name, dh.getContent(), function () {
				onsuccess();
		...
		// download(load, url, files, function (data) {
	...
	var nativeLoadHandler = function () {
		...
		if (nativeLoad.handleCount > 0) {
			throw new Error("nativeLoadHandler Logic Error11");
		}
		...
		for (var i = 0; i < nativeLoad.handleCount; ++i) {
			...
			var pathWhich = nativeLoad.filesWitch.pop();
			(function (name, okCB, errCB) {
				if (pathWhich === "indexedDB") {
					store.sread(localStore, name, function (s) {
						handleWait();
						okCB(s);
					}, function (e) {
						handleWait();
					...
					JSVM.getDownRead(depend.domains[0] + depend.rootPath() + name, name, function (s) {
						handleWait();
						okCB(s);
					}, function (e) {
						handleWait();
	...
	var localLoad = function (load /*:Load*/, arr) {
		...
		for (i = files.length - 1; i >= 0; i--) {
			...
			if (info.sign === sign) {
				...
				load.fileMap[name] = 0;
				if (s === sign) {
					nativeLoad.filesWitch.push("indexedDB");
				} else {
					nativeLoad.filesWitch.push("nativeLoad");
				}
	...
	// // 下载
	// var download = function (load, path, files, onsuccess, onerror, onprocess, err, retry) {
	// 	if (retry >= depend.httpDomains.length) {
	// 		return onerror(err);
	// 	}
	// 	load.ajax[path] = ajax.get(depend.httpDomains[retry || 0] + path, undefined, undefined, undefined, ajax.RESP_TYPE_BIN, onsuccess, function (err) {
	// 		download(load, path, files, onsuccess, onerror, onprocess, err, retry === undefined ? 0 : retry + 1);
	// 	}, onprocess);
	// };
	...
	// var savefile = function (name, data, info, successCallback, errorCallback) {
		// store.write(localStore, name, base64js.fromByteArray(data), function () {
...
pi_modules.commonjs.exports = (function () {
	...
	module.require = function (modNames, fileMap, successCallback, errorCallback, processCallback) {
		...
		for (i = len = modSet.length - 1; i >= 0; i--) {
			mod = modSet[i]; 
			// TODO rpc里面如果有require此处JSON.stringify不知道为啥会执行mod里面的代码 bitcore-lib会报错
			// var a = JSON.stringify(mod);
			// console.log(a);
			// TODO rpc里面如果有require此处JSON.stringify不知道为啥会执行mod里面的代码 bitcore-lib会报错
			// var a = JSON.stringify(mod);
			// console.log(a);
	...
	var loadJS = function (cfg) {
		var initFail = function (url, err) {
			err ? console.log(err + ", " + url) : "";
		};
		setTimeout(() => cfg.data.runScript(), 10);
	};
	// 模块定义成功，通知所有正在等待的加载器
...
pi_modules.update.exports = (function () {
	...
	module.UPDATE_FLAG_LAST_ERROR = 4; // 取不到最新jsindex.js，同时上次没更新完成
	...
	module.checkUpdate = function (callback) {
		...
		var nextFunc = function (indexJSStr) {
			...
			console.log("JSVM update, old jsindex.js version = " + oldVersion.join("."));
			// 检查是否需要更新
			checkNeedUpdate(oldVersion, callback);
		}

		// 获取document的indexjs
		var dataHandle = JSIntercept.getBootFile("jsindex.js");
		if (dataHandle) {
			var indexJSStr = dataHandle.getContent();
			console.log("JSVM update, get jsindex.js from app's directory");
			nextFunc(indexJSStr);
		} else {
			// 从assets的jsindex.js取到老版本号
			console.log("JSVM update, get index.js from assets======",localDomain);
			nextFunc(JSVM.Boot.loadJS(localDomain, rootPath + bootDir + "jsindex.js").getContent());
		}
	};
	...
	module.update = function (updateProcessCb, updateFinishCb) {
		...
		// 设置标记，开始更新
		// JSIntercept.startUpdate("update.flag", "true", function () {
		// console.log("JSVM update, now start update");
	...
	module.reload = function () {
		if (!inApp) return;
		// console.log("JSVM update, now reload");
		JSIntercept.restartJSVM();
	};
	...
	// 从jsindex.js的版本号检查是否需要更新
	function checkNeedUpdate(oldVersion, okCB) {

		remoteVersion = localVersion = oldVersion;
		// 取服务器的jsindex.js
		ajax.get(httpDomain + rootPath + bootDir + "jsindex.js?" + Math.random(), undefined, undefined, undefined, undefined, function (indexJSStr) {
			...
			console.log("JSVM update, new index.js version = " + newVersion.join("."));
			...
			console.log("JSVM update, depend version = " + dependVerison);

			let appVersion = pi_modules.appUpdate.exports.getLocalVersion();
			if (compareVersion(dependVerison, appVersion) > 0) {
				console.log("JSVM update,app version too low, dependVerison = ", dependVerison, ", appVersion = ", appVersion, ", isForceUpdate = " + isForceUpdate);
				...
				if (isForceUpdate) {
					...
					// console.log("appversion is too low to update")
			...
			console.log("JSVM update,lastUpdate = " + isForceUpdate + ",oldVersion=" + oldVersion + ", newVersion=" + newVersion);
			...
			// console.log("JSVM update, can't get jsindex.js from server, needUpdate = " + needUpdate);
			if (needUpdate) {
				// 获取不到jsindex.js，同时上次没更新完毕，报错
	...
	function startUpdate(updateProcessCb, updateFinishCb) {
		...
		load.initLocal(function () {
			// console.log("JSVM update, load.initLocal ok");
			var url = httpDomain + rootPath + ".depend?" + Math.random();
			//下载depend
			ajax.get(url, undefined, undefined, undefined, undefined, function (data) {
				dependFileData = data;
				JSVM.Boot.saveDepend(data);
				...
				downloadNewFiles(dependData, function (files) {
					if (files) {
						// console.log("JSVM update, downloadNewFiles ok, files.length = " + files.length);
					}
					// JSVM.Boot.saveDepend()
					// saveBootFiles(files, function () {
					// 	// 清除标记
					// 	// console.log("JSVM update, saveBootFiles ok");
					// 	JSIntercept.saveFile("update.flag", "false", function () {
							// console.log("JSVM update, save update.flag = false ok");
					updateFinishCb();
						// });
					// });
				}, function (e) {
	...
	// 	++saveCount;
	// 	var dependArrayBuffer = butil.utf8Encode(dependFileData);
	// 	bootFiles[rpath + ".depend"] = dependArrayBuffer;
	// 	if (newIndexJSStr) {
	// 		++saveCount;
	// 		bootFiles[rpath + bootDir + "jsindex.js"] = butil.utf8Encode(newIndexJSStr);
	// 	}
	...
	// 		// console.log("JSIntercept.saveFile, path = " + path + ", size = " + str.length, str);
	...
	function downloadNewFiles(dependData, okCB, failCB, updateProcessCb) {
		var downloads = [];
		//如果jsindex.js下载下来了，存一下
		if(newIndexJSStr){
			JSVM.Boot.saveIndexJS(newIndexJSStr);
		}
		for (var i = 0; i < dependData.length; ++i) {
			var info = dependData[i];
			// 本地没有的才需要下载
			if (!load.isLocal(info.path) && info.size > 0) {
				// 当jsindex.js还没有下载的时候，必须进去
				if (!newIndexJSStr || info.path !== bootDir + "jsindex.js") {
		...
		// console.log("JSVM update, downloadNewFiles, downloads.length = " + downloads.length);
	...
	function getIndexVersion(indexJS) {
		...
		if (!result || result.length !== 5) {
			// console.log("jsindex.js must have version at first line, format like: // !version=1.0.0.100916")
		}
		return [result[1], result[2], result[3], result[4]];
	}

	// 取jsindexjs的depend版本号
	function getDependVersion(indexJS) {
		...
		if (!versions || versions.length !== 4) {
			// console.log("jsindex.js must have depend_version")
		}
...
pi_modules.appUpdate.exports = (function () {
	...
	module.needUpdate = function (cb) {
		...
		var next = function () {
			ajax.get(configURL, undefined, undefined, undefined, ajax.RESP_TYPE_TEXT, function (r) {
				console.log(r)
...
var onerror = winit.debug ? undefined : (function () {
	var map = {},
		count = function (e) {
			var v = map[e] || 0,
				c = 1;
