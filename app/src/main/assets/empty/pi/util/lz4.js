_$define("pi/util/lz4", function (require, exports, module){
"use strict";
/**
 * 编译asm.js命令
 *
...
const lz4_asm_1 = require("./lz4_asm");
/**
 * 压缩
 * @return Uint8Array，压缩后的数据，用完后需要调用free释放内存
 */
exports.compress = (src) => {
    ...
    if (compressSize <= 0) {
        // 压缩出错
        lz4_asm_1.LZ4ASM._free(compressHeap.byteOffset);
        return;
    }
 ...
 * @return Uint8Array，压缩后的数据，用完后需要调用free释放内存
 */
exports.decompress = (src, decompressSize) => {
    ...
    if (realSize <= 0) {
        lz4_asm_1.LZ4ASM._free(decompressHeap.byteOffset);
        return;
    }
 ...
 */
exports.free = (heap) => {
    lz4_asm_1.LZ4ASM._free(heap.byteOffset);
};
...
const LZ4_compress_default = lz4_asm_1.LZ4ASM.cwrap('LZ4_compress_default', 'number', ['array', 'number', 'number', 'number']);
/*! LZ4_decompress_safe() :
...
// tslint:disable-next-line:variable-name
const LZ4_decompress_safe = lz4_asm_1.LZ4ASM.cwrap('LZ4_decompress_safe', 'number', ['array', 'number', 'number', 'number']);
/*!
LZ4_compressBound() :
...
// tslint:disable-next-line:variable-name
const LZ4_compressBound = lz4_asm_1.LZ4ASM.cwrap('LZ4_compressBound', 'number', ['number']);
/**
 * 创建属于asmjs的堆对应的U8Array，用完后需要显示调用free释放内存
 * @param size 分配大小
 */
const newU8Heap = (size) => {
    const ptr = lz4_asm_1.LZ4ASM._malloc(size);
    const r = new Uint8Array(lz4_asm_1.LZ4ASM.HEAPU8.buffer, ptr, size);
    return r;
};
});
