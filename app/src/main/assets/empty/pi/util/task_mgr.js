 ...
 * @return    定时器的引用
 */
exports.setTimer = (callback, args, ms, count, delay) => {
    count = count || -1;
...
exports.set = (func, args, priority, type) => {
    if (func) {
 ...
 * @example
 */
exports.getPrioritySize = (priority, type) => {
    return taskPool.getPrioritySize(priority, type);
};
// 函数调用计时
exports.callTime = (func, args, name, start) => {
...
// 任务池
const taskPool = new task_pool_1.TaskPool();
// 帧回调是否可用
let intervalWaitRef;
// 任务执行函数
const exec = () => {
    const frame = frame_mgr_1.getGlobal();
    let nextTime = frame.getNextTime() - frameTimeout;
    let start = time_1.now();
    if (nextTime < start) {
        nextTime = start + 1;
    }
...
const requestIdle = self.requestIdleCallback1 || function (callback) { setTimeout(callback, 1); };
const cancelIdle = self.cancelIdleCallback1 || clearTimeout;
});
