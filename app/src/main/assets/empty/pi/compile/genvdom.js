...
const preorder = (syntax, parent) => {
    const index = funcStrIndex;
    const funcs = seekFunc(syntax, parent);
    const childs = funcs.child();
    const childNodes = [];
    funcStrIndex++;
    ...
    const node = funcs.node(childNodes);
...
const genAutoFunc = (operator) => {
    return (syntax, parent) => {
        return Object.assign({}, defaultParse, { child: () => {
                const childs = [];
...
const gentTagPreFunc = (tagName, syntax, id) => {
    return (node) => {
        const ssid = syntax.sid !== undefined ? syntax.sid : id;
...
const genSignleTagFunc = (tagName) => {
    return (syntax, parent) => {
        return Object.assign({}, defaultParse, { child: () => syntax.right, node: (childs) => {
...
const tagNodeFunc = (tagName, childs) => {
    const node = new ParserNode();
    sumChildHash(node, childs);
    calTagHash(node, tagName);
    return node;
};
const tagFunc = (syntax, parent) => {
    return Object.assign({}, defaultParse, { child: () => [syntax.right[1], syntax.right[2]], node: (childs) => {
            const node = containScript(childs);
            calTagHash(node, syntax.right[0].value); // 计算taghash
...
const textFunc = (syntax, parent) => {
    return Object.assign({}, defaultParse, { node: (childs = null) => {
...
const jarrFunc = (syntax, parent) => {
    return Object.assign({}, defaultParse, { child: () => {
            ...
            calHash(childs, node);
            return node;
        }, pre: () => `_$temp=node;{let _$parent = _$temp;let node = [];`, suf: (node) => {
            let str = ``;
            for (let i = 0; i < syntax.right.length; i++) {
                let v;
...
const jstrFunc = (syntax, parent) => {
    return Object.assign({}, defaultParse, { pre: (node) => `jvalue = "${syntax.right[0].value}";`, node: (childs) => {
            const node = new ParserNode();
...
const defFunc = (syntax, parent) => {
    return Object.assign({}, defaultParse, { child: () => [syntax.right[0]], node: (childs) => {
            const node = new ParserNode();
...
const bracketFunc = (syntax, parent) => {
    return Object.assign({}, defaultParse, { child: () => isBuildIn(syntax.right[0]) ? [] : [syntax.right[0]], node: (childs) => {
            const node = new ParserNode();
...
const jsExprFunc = (syntax, parent) => {
    ...
    return Object.assign({}, defaultParse, { child: () => isBuildIn(syntax.right[0]) ? [] : syntax.right, node: (childs) => {
            const node = new ParserNode();
            if (!isBuildIn(syntax.right[0])) {
            ...
            if (p.type === 'attr') {
                ...
                isBuildIn(syntax.right[0]) ?
                    str = `node[${syntax.index}] = ${syntax.right[0].value};` : str = `node[${syntax.index}] = ${node.str};`;
            }
...
const ifFunc = (syntax, parent) => {
    return Object.assign({}, defaultParse, { child: genifelseifChildFunc(syntax), node: genifelseifNodeFunc(`if`, syntax), pre: (node) => node.str });
};
const elseifFunc = (syntax, parent) => {
    return Object.assign({}, defaultParse, { child: genifelseifChildFunc(syntax), node: genifelseifNodeFunc(`else if`, syntax), pre: (node) => node.str });
};
const elseFunc = (syntax, parent) => {
    return Object.assign({}, defaultParse, { child: () => syntax.right, pre: (node) => `else` });
};
...
const jscontinueFunc = (syntax, parent) => {
    return Object.assign({}, defaultParse, { pre: (node) => `continue;` });
};
const regularFunc = (syntax, parent) => {
    return Object.assign({}, defaultParse, { node: (node) => syntax.value });
};
const jsbreankFunc = (syntax, parent) => {
    return Object.assign({}, defaultParse, { pre: (node) => `break;` });
};
...
// 汇总静态hash
const calChildsHash = (childs, node) => {
    for (let i = 0; i < childs.length; i++) {
...
const attrscriptFunc = (syntax, parent) => {
    return Object.assign({}, defaultParse, { node: (childs) => {
            const node = new ParserNode();
            node.cs = true;
            calHash(childs, node);
            return node;
...
const jswhileFunc = (syntax, parent) => {
    return Object.assign({}, defaultParse, { child: () => genRightBuiltIn(syntax), node: (childs) => {
            const node = new ParserNode();
            node.str = `while(`;
            if (isBuildIn(syntax.right[0])) {
                node.str += syntax.right[0].value + ')';
...
const isBuildIn = (syntax) => syntax.type === 'string' || syntax.type === 'number' || syntax.type === 'bool'
