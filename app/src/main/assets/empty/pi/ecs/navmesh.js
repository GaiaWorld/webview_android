...
const aabb_1 = require("../math/aabb");
const astar_1 = require("../math/astar");
 ...
 */
class Polygon {
    constructor(navMesh, indexs, adj, cost, index) {
        this.indexs = indexs;
        this.adjacency = adj;
    ...
    // 多边形是否包含点
    containsPoint(point) {
        let vec1 = new vector3_1.Vector3().subVectors(this.navMesh.vertexs[this.indexs[this.indexs.length - 1]], point);
        let vec2 = new vector3_1.Vector3().subVectors(this.navMesh.vertexs[this.indexs[0]], point);
        let ret1 = new vector3_1.Vector3().crossVectors(vec1, vec2);
        let vec3 = new vector3_1.Vector3();
        let ret2 = new vector3_1.Vector3();
        let temp;
...
class LSegment {
    constructor(navMesh, p1, p2, poly1, poly2) {
        this.p1 = p1;
        this.p2 = p2;
        this.navMesh = navMesh;
...
// 起点和终点
class SEPoint {
    constructor(navMesh, center, poly) {
        this.center = center;
        this.navMesh = navMesh;
        this.poly = poly;
    }
...
class NavMesh {
     ...
     */
    findPath(start, end, radio = 0) {
        const newEnd = new vector3_1.Vector3();
    ...
    funnel(node, start, end, radio = 0) {
        ...
        let radioV;
        let adjLine; // 相邻多边形共线
        let lastLine; // lastPoint指向end的向量
...
// 取到两多边形的共边
const getAdjLine = (navMesh, poly1, poly2) => {
...
// 取到两边所在的多边形
const getAdjPoly = (navMesh, l1, l2) => {
...
// 检查三个向量是否按顺序排布
const vIsOrder = (v1, v2, v3) => {
    const cross1 = new vector3_1.Vector3().crossVectors(v2, v1);
    const cross2 = new vector3_1.Vector3().crossVectors(v2, v3);
