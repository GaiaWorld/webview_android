 ...
 * 协议块处理
 */
const erl_term_1 = require("./erl_term");
const util_1 = require("./util");
const xxtea64_1 = require("./xxtea64");
const zlib_1 = require("./zlib");
const util_2 = require("../../bigint/util");
const random = (seed) => {
...
const writeInt64 = (jbuf, msg) => {
    ...
    let h;
    b = util_1.createU8ArrayView(jbuf.buf, jbuf.offset, len);
    const temp = Math.abs(msg) / Math.pow(2, 32);
    h = parseInt(temp, 10);
    b[6] = (h >>> 16) & 0xff;
    b[5] = (h >>> 8) & 0xff;
    b[4] = h & 0xff;
...
const writeString = (jbuf, str) => {
    const len = util_1.utf8Length(str);
    writeInt8(jbuf, len);
...
const mDecode = (msg, jbuf, offset, length) => {
    let k;
    let t;
    let len = erl_term_1.readInt8(readU8Array(jbuf, 1));
    const param = {};
    let dbuf;
    let tempArray;
    ...
    while (jbuf.offset < length) {
        ...
        else if (t === erl_term_1.TAG.BIG_INT_TAG) {
            tempArray = readU8Array(jbuf, len - 1);
            tempArray.offset = 0;
            param[k] = util_2.bufferToU64(new Uint8Array(tempArray.buf), false);
